...
const TEST_DB_PATH = path.join(TEST_OUTPUT_DIR, 'test-sqlite-export.db');
...
describe('SQLite Export Migration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log('✅ Connected to test database for SQLite export tests');
    } catch (err) {
  ...
  describe('Basic SQLite Export', () => {
    test('should export Boulder region to SQLite', async () => {
      // Skip if no test database available
      if (!process.env.TEST_PGHOST && !process.env.PGHOST) {
        console.log(`⏭️  Skipping boulder test - no test database available`);
        return;
      }
    ...
    test('should export Seattle region to SQLite', async () => {
      // Skip if no test database available
      if (!process.env.TEST_PGHOST && !process.env.PGHOST) {
        console.log(`⏭️  Skipping seattle test - no test database available`);
        return;
      }
