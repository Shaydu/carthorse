...
export interface SubdivisionConfig {
  region: string;
  maxTrailsPerChunk?: number; // defaults to 1000
  overlapPercentage?: number; // defaults to 0.1 (10% overlap)
  minChunkSize?: number; // minimum trails per chunk, defaults to 100
}

export class BboxSubdivider {
   ...
   */
  async subdivideRegion(config: SubdivisionConfig): Promise<BboxSubdivision[]> {
    const {
      region,
      maxTrailsPerChunk = 1000,
    ...
    const subdivisions: BboxSubdivision[] = [];
   ...
   */
  async processSubdivision(
    subdivision: BboxSubdivision, 
    stagingSchema: string,
    region: string
  ): Promise<{ success: boolean; trailCount: number; errors: string[] }> {
   ...
   */
  async subdivideStagingData(
    sourceStagingSchema: string,
    maxTrailsPerChunk: number = 500,
    minChunkSize: number = 50
  ): Promise<BboxSubdivision[]> {
   ...
   */
  async processStagingSubdivision(
    subdivision: BboxSubdivision,
    sourceStagingSchema: string,
    targetStagingSchema: string
  ): Promise<{ success: boolean; trailCount: number; errors: string[] }> {
   ...
   */
  async cleanupSubdivisions(subdivisions: BboxSubdivision[]): Promise<void> {
 ...
 */
export function createBboxSubdivider(pgClient: Pool | Client): BboxSubdivider {
