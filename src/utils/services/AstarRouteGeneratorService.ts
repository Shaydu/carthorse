...
export interface AstarRouteGeneratorConfig {
...
export class AstarRouteGeneratorService {
  ...
  constructor(
    ...
    private config: AstarRouteGeneratorConfig
  ) {
    ...
    for (const tol of tolerances) {
      for (const start of nodes) {
        ...
        for (const r of reachable) {
          ...
          const rows = await this.sqlHelpers.executeAstarRouting(this.config.stagingSchema, start.id, endNode);
          let seq = 1;
