 ...
 */
export async function runPostNodingVertexMerge(
  pgClient: Pool,
  stagingSchema: string,
  toleranceMeters: number
): Promise<{ mergedVertices: number; remappedSources: number; remappedTargets: number; deletedOrphans: number }> {
  const tolDegrees = toleranceMeters / 111_320; // approximate for small distances in EPSG:4326
