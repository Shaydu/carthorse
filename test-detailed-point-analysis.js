...
async function testDetailedPointAnalysis() {
  ...
  try {
    ...
    // Detailed analysis of each intersection point individually
    const detailedAnalysisQuery = `
      WITH trail_pairs AS (
        SELECT 
      ...
      SELECT 
        ...
        -- Test different tolerance values for each point individually
        CASE 
        ...
        -- Boolean flags for the fixed logic
        (ST_DWithin(point_geom, trail1_start, 1.0) 
         OR ST_DWithin(point_geom, trail1_end, 1.0)
         OR ST_DWithin(point_geom, trail2_start, 1.0)
         OR ST_DWithin(point_geom, trail2_end, 1.0)) as is_endpoint_intersection_1_0m,
        (NOT ST_DWithin(point_geom, trail1_start, 1.0) 
         AND NOT ST_DWithin(point_geom, trail1_end, 1.0)
         AND NOT ST_DWithin(point_geom, trail2_start, 1.0)
         AND NOT ST_DWithin(point_geom, trail2_end, 1.0)) as is_middle_intersection_1_0m
      FROM intersection_points
      ORDER BY trail1_name, trail2_name, point_path;
    `;

    const result = await client.query(detailedAnalysisQuery);
    
    console.log('\n=== DETAILED POINT-BY-POINT ANALYSIS ===');
    console.log('Analyzing each intersection point individually:\n');
    
    for (const row of result.rows) {
      console.log(`📍 Point ${row.point_path}: ${row.point_wkt}`);
      console.log(`   Coordinates: ${row.lon}, ${row.lat}, ${row.elevation}`);
      ...
      console.log(`   Is endpoint intersection (1.0m): ${row.is_endpoint_intersection_1_0m}`);
      console.log(`   Is middle intersection (1.0m): ${row.is_middle_intersection_1_0m}`);
      console.log('---');
    }

    // Now test the aggregate logic that the service uses
    console.log('\n=== AGGREGATE LOGIC ANALYSIS ===');
    const aggregateAnalysisQuery = `
      WITH trail_pairs AS (
        SELECT 
          t1.app_uuid as trail1_uuid,
          t1.name as trail1_name,
      ...
      intersection_analysis AS (
        SELECT 
          ...
          trail2_start,
          trail2_end,
          -- Check if ANY intersection points are near trail endpoints (within 1.0 meter)
          EXISTS(
            SELECT 1 FROM (
    ...
    const aggregateResult = await client.query(aggregateAnalysisQuery);
