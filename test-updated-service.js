...
async function testUpdatedService() {
  ...
  try {
    ...
    // Test the updated splitting logic
    console.log('\n🔧 Testing updated splitting logic...');
    ...
    // Test splitting trail 1 with the updated logic
    console.log('\n🔨 Testing updated splitting logic for Trail 1...');
    const pointGeometries = intersectionPoints.map(p => p.point);
    const collectQuery = `ST_Collect(${pointGeometries.map((_, i) => `$${i + 2}`).join(', ')})`;
    
    const splitTrail1Result = await pool.query(`
      SELECT ST_Split($1::geometry, ${collectQuery}) as split_geom
    `, [intersection.trail1_geom, ...pointGeometries]);
    ...
    console.log('\n🔨 Testing updated splitting logic for Trail 2...');
    const splitTrail2Result = await pool.query(`
      SELECT ST_Split($1::geometry, ${collectQuery}) as split_geom
    `, [intersection.trail2_geom, ...pointGeometries]);
    ...
    console.log('\n🎉 SUCCESS: The updated MultipointIntersectionSplittingService logic works correctly!');
