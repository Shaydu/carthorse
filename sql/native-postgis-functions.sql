...
-- Function to split trails at intersections using native PostGIS ST_Node
CREATE OR REPLACE FUNCTION native_split_trails_at_intersections(
    staging_schema text,
    trails_table text
) RETURNS integer AS $$
DECLARE
...
BEGIN
    ...
    -- Insert split trails using native PostGIS ST_Node and ST_Dump
    dyn_sql := format($f$
        ...
        SELECT 
            ...
            seg.segment_number,
            t.app_uuid || '-seg' || seg.segment_number as app_uuid,
        ...
        CROSS JOIN LATERAL (
            SELECT 
                row_number() OVER (PARTITION BY t.id ORDER BY dump.path) as segment_number,
                dump.geom as geometry
            FROM ST_Dump(ST_Node(ST_Force3D(t.geo2))) dump
            WHERE t.geo2 IS NOT NULL AND ST_IsValid(t.geo2)
        ) seg
        WHERE seg.geometry IS NOT NULL AND ST_GeometryType(seg.geometry) = 'ST_LineString'
    $f$, staging_schema, staging_schema, trails_table);
    
    EXECUTE dyn_sql;
    
    -- Get count of split trails
    EXECUTE format('SELECT COUNT(*) FROM %I.split_trails', staging_schema) INTO split_count;
