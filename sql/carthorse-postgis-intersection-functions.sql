...
-- Splits trails ONLY at actual intersections using ST_Node()
...
BEGIN
    ...
    EXECUTE format($f$
        ...
        -- Split trails ONLY at actual intersections using ST_Node()
        split_trails AS (
            SELECT 
                ...
                t.bbox_min_lat,
                t.bbox_max_lat,
                -- Use ST_Node() to split ONLY at actual intersections
                (ST_Dump(ST_Node(t.geometry))).geom as split_segment,
                ROW_NUMBER() OVER (PARTITION BY t.id ORDER BY (ST_Dump(ST_Node(t.geometry))).path) as segment_number
            FROM trail_geometries t
            WHERE ST_Length(t.geometry::geography) > 0.1 -- Only process trails longer than 100m
        )
        SELECT 
