-- Simple Intersection Detection Routing Functions for Carthorse
-- Uses the exact ST_Intersects algorithm that found 176 intersections

-- Function to create routing graph using simple intersection detection
CREATE OR REPLACE FUNCTION simple_intersection_routing(
    staging_schema text,
    trails_table text,
...
BEGIN
    ...
    -- Step 1: Find all intersection points using the simple ST_Intersects algorithm
    EXECUTE format($f$
...
CREATE OR REPLACE FUNCTION simple_intersection_stats(
    staging_schema text
) RETURNS TABLE (
    total_nodes integer,
...
BEGIN
    ...
    EXECUTE format($f$
        SELECT 
            ...
            (SELECT COUNT(*) FROM %I.%I t1 JOIN %I.%I t2 ON t1.id < t2.id WHERE ST_Intersects(t1.geometry, t2.geometry)) as intersection_pairs
        FROM %I.routing_nodes
    $f$, staging_schema, staging_schema, 'boulder_valley_ranch_subset', staging_schema, 'boulder_valley_ranch_subset', staging_schema);
END;
$$ LANGUAGE plpgsql; 