-- Fix for find_routes_recursive_configurable function to handle UUID comparisons
-- This fixes the UUID vs integer comparison issues
...
BEGIN
    ...
    RETURN QUERY EXECUTE format($f$
        WITH RECURSIVE route_search AS (
            ...
            -- Create a mapping of UUID to integer for consistent comparisons
            SELECT 
                ...
                (row_number() OVER (ORDER BY id))::integer as node_int_id  -- Integer ID for comparisons
            FROM %I.routing_nodes
            
            UNION ALL
            
            -- Recursively explore connected nodes
            -- Use correct column names (source/target) and handle UUIDs properly
            SELECT 
                rs.start_node,
                e.target_int_id as current_node,
                e.target_int_id as end_node,
                rs.path || e.target_int_id,
                rs.edges || e.edge_int_id,
            ...
            JOIN (
                -- Create a subquery that maps UUIDs to integers for edges
                SELECT 
                    e.source,
                    e.target,
                    e.id as edge_id,
                    ...
                    e.length_km,
                    e.elevation_gain,
                    (row_number() OVER (ORDER BY e.target))::integer as target_int_id,
                    (row_number() OVER (ORDER BY e.id))::integer as edge_int_id
                FROM %I.routing_edges e
            ) e ON rs.original_node_id = e.source  -- Use source/target columns
            WHERE rs.depth < $1  -- Limit depth to prevent infinite loops
              AND e.target_int_id != ALL(rs.path)  -- Avoid cycles using integer IDs
