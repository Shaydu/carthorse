-- Fix for find_routes_recursive_configurable function to handle UUIDs
-- This fixes the data type issues for staging schema routing
...
BEGIN
    ...
    RETURN QUERY EXECUTE format($f$
        WITH RECURSIVE route_search AS (
            ...
            -- Convert UUID to integer for compatibility
            SELECT 
                ...
                id as original_id  -- Keep original UUID for reference
            FROM %I.routing_nodes
            
            UNION ALL
            
            -- Recursively explore connected nodes
            -- FIXED: Use correct column names and handle UUIDs
            SELECT 
                rs.start_node,
                ...
                e.target as original_id
            FROM route_search rs
            JOIN %I.routing_edges e ON rs.original_id = e.source  -- FIXED: was e.from_node_id
