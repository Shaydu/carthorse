-- Fix for find_routes_recursive_configurable function
-- This fixes the column name issues for staging schema routing
...
BEGIN
    ...
    RETURN QUERY EXECUTE format($f$
        WITH RECURSIVE route_search AS (
            ...
            -- FIXED: Use correct column names (source/target instead of from_node_id/to_node_id)
            SELECT 
                rs.start_node,
                e.target as current_node,  -- FIXED: was e.to_node_id
                e.target as end_node,      -- FIXED: was e.to_node_id
                rs.path || e.target,       -- FIXED: was e.to_node_id
                rs.edges || e.id,
            ...
            JOIN %I.routing_edges e ON rs.current_node = e.source  -- FIXED: was e.from_node_id
            WHERE rs.depth < $1  -- Limit depth to prevent infinite loops
              AND e.target != ALL(rs.path)  -- FIXED: was e.to_node_id, avoid cycles
