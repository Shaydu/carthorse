...
CREATE OR REPLACE FUNCTION detect_trail_intersections(
    trails_schema text,
    trails_table text,
    intersection_tolerance_meters float DEFAULT get_intersection_tolerance()
) RETURNS TABLE (
...
BEGIN
    RETURN QUERY EXECUTE format('
        ...
        ST_Force3D(ST_EndPoint(t1.noded_geom)) as intersection_point_3d,
            ...
            FROM noded_trails t1
            JOIN noded_trails t2 ON (t1.id < t2.id)
            WHERE ST_DWithin(ST_EndPoint(t1.noded_geom), ST_EndPoint(t2.noded_geom), GREATEST($1, get_simplify_tolerance()))
        ),
...
BEGIN
    ...
    EXECUTE format('
        ...
        node_matches AS (
            ...
            LEFT JOIN %I.routing_nodes rn1 ON ST_DWithin(se.start_point, rn1.geo2, get_simplify_tolerance())
            LEFT JOIN %I.routing_nodes rn2 ON ST_DWithin(se.end_point, rn2.geo2, get_simplify_tolerance())
