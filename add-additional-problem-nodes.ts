...
async function addAdditionalProblemNodes() {
  ...
  try {
    ...
    // Define the additional problematic coordinates
    const additionalCoordinates = [
      {
        lng: -105.306075,
        lat: 40.00383,
        elevation: 2108.483398438,
        reason: "Should be snapped to nearest path and split into degree 3 intersection"
      },
      {
        lng: -105.305985,
        lat: 40.00365,
        elevation: 2109.218505859,
        reason: "Should be snapped to nearest path and split into degree 3 intersection"
      },
      {
        lng: -105.32304,
        ...
        reason: "Snap, split and degree 3 node"
      },
      {
        lng: -105.297435,
        lat: 40.03497,
    ...
    console.log(`\n🔍 Processing ${additionalCoordinates.length} additional problem coordinates...`);
    ...
    const pytorchData = {
      ...
      metadata: {
        ...
        num_classes: 3,
        schema: schema,
        additional_problem_nodes_added: {
          existing_nodes_updated: foundNodes.length,
          new_coordinates_identified: newNodes.length,
          total_coordinates_processed: additionalCoordinates.length
        },
        generated_at: new Date().toISOString(),
        additional_problem_nodes: {
    ...
    const outputPath = path.join('test-output', `graphsage-data-${schema}-with-additional-problems-${timestamp}.json`);
...
addAdditionalProblemNodes().catch(console.error);
