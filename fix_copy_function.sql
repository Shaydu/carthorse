-- Fixed copy function without geometry_text and geometry_hash - V6 (Proper fix)
CREATE OR REPLACE FUNCTION copy_and_split_trails_to_staging_native_v6(
    staging_schema text, 
    source_table text, 
    region_filter text, 
...
BEGIN
    ...
    RETURN QUERY SELECT 
        ...
        true, 
        format('Successfully copied and split %s trails with %s intersections (v6)', split_count_var, intersection_count_var);

EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 0, 0, 0, false, format('Error during copy and split (v6): %s', SQLERRM);
END;
$$ LANGUAGE plpgsql; 