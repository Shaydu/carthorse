-- Fixed copy function that goes back to original behavior - V10
-- Generate new UUIDs for all segments (original + split) to avoid collisions
CREATE OR REPLACE FUNCTION copy_and_split_trails_to_staging_native_v10(
    staging_schema text, 
    source_table text, 
    region_filter text, 
...
BEGIN
    ...
    -- This approach generates new UUIDs for ALL segments (original + split)
    EXECUTE format($f$
    ...
    RETURN QUERY SELECT 
        ...
        true as success,
        format('Successfully copied and split %s trails into %s segments with %s intersections (v10)', 
...
EXCEPTION WHEN OTHERS THEN
    ...
    RETURN QUERY SELECT 
        0, 0, 0, false, 
        format('Error during copy and split (v10): %s', SQLERRM) as message;
