-- Fixed copy function that follows the working pattern from July 31st - V11
-- Generate new UUIDs for all segments (original + split) to avoid collisions
CREATE OR REPLACE FUNCTION copy_and_split_trails_to_staging_native_v11(
    staging_schema text, 
    source_table text, 
    region_filter text, 
...
BEGIN
    ...
    RETURN QUERY SELECT 
        ...
        true as success,
        format('Successfully copied and split %s trails into %s segments with %s intersections (v11)', 
...
EXCEPTION WHEN OTHERS THEN
    ...
    RETURN QUERY SELECT 
        0, 0, 0, false, 
        format('Error during copy and split (v11): %s', SQLERRM) as message;
