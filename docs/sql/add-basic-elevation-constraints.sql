...
ALTER TABLE trails
  ...
  ADD CONSTRAINT trails_valid_geometry CHECK (ST_IsValid(geometry)),
  ADD CONSTRAINT trails_3d_geometry CHECK (ST_NDims(geometry) = 3),
  ADD CONSTRAINT trails_min_points CHECK (ST_NPoints(geometry) >= 2); 