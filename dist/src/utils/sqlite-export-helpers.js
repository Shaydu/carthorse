...
exports.CARTHORSE_SCHEMA_VERSION = void 0;
exports.createSqliteTables = createSqliteTables;
exports.insertTrails = insertTrails;
exports.insertRoutingNodes = insertRoutingNodes;
exports.insertRoutingEdges = insertRoutingEdges;
exports.insertRegionMetadata = insertRegionMetadata;
exports.buildRegionMeta = buildRegionMeta;
exports.insertSchemaVersion = insertSchemaVersion;
exports.getSchemaVersionFromDatabase = getSchemaVersionFromDatabase;
exports.insertRouteRecommendations = insertRouteRecommendations;
exports.insertRouteTrails = insertRouteTrails;
const schema_version_reader_1 = require("./schema-version-reader");
// Schema version for SQLite exports - read directly from SQL file
exports.CARTHORSE_SCHEMA_VERSION = (0, schema_version_reader_1.getCurrentSqliteSchemaVersion)();
/**
 ...
 */
function createSqliteTables(db, dbPath) {
  ...
  `);
    ...
    const hasSourceColumn = edgesTableInfo.some((col) => col.name === 'source');
    const hasTargetColumn = edgesTableInfo.some((col) => col.name === 'target');
    if (!hasSourceColumn || !hasTargetColumn) {
        ...
        console.error('[SQLITE] Available columns:', edgesTableInfo.map((col) => col.name));
 ...
 */
function insertTrails(db, trails, dbPath) {
  ...
  `);
    const insertMany = db.transaction((trails) => {
        for (const trail of trails) {
            ...
            if (source_tags && typeof source_tags !== 'string')
                source_tags = JSON.stringify(source_tags);
            ...
            insertStmt.run(trail.app_uuid || null, trail.name || null, trail.region, // Region should never be null - required field
            trail.osm_id || null, trail.osm_type || null, trail.trail_type || null, trail.surface_type || null, // v14 schema uses surface_type
            difficulty, geojson, trail.length_km, // No fallback - must be present
            ...
            trail.bbox_min_lng ?? null, trail.bbox_max_lng ?? null, trail.bbox_min_lat ?? null, trail.bbox_max_lat ?? null, trail.created_at ? (typeof trail.created_at === 'string' ? trail.created_at : trail.created_at.toISOString()) : new Date().toISOString(), trail.updated_at ? (typeof trail.updated_at === 'string' ? trail.updated_at : trail.updated_at.toISOString()) : new Date().toISOString());
        }
    });
    insertMany(trails);
 ...
 */
function insertRoutingNodes(db, nodes, dbPath) {
  ...
  `);
    ...
    const insertMany = db.transaction((nodes) => {
        for (const node of nodes) {
            try {
                const result = insertStmt.run(node.node_uuid || null, node.lat || null, node.lng || null, node.elevation || null, node.node_type || 'intersection', node.connected_trails || null, node.created_at ? (typeof node.created_at === 'string' ? node.created_at : node.created_at.toISOString()) : new Date().toISOString());
                if (result.changes > 0) {
                    insertedCount++;
                }
            }
 ...
 * @param dbPath Optional database path for logging
 */
function insertRoutingEdges(db, edges, dbPath) {
    ...
    const insertMany = db.transaction((edges) => {
        for (const edge of edges) {
            try {
                ...
                const result = insertStmt.run(edge.source, edge.target, edge.trail_id, edge.trail_name, // Now required
                edge.length_km, // Changed from distance_km
                edge.elevation_gain || null, edge.elevation_loss || null, edge.geojson, edge.created_at ? (typeof edge.created_at === 'string' ? edge.created_at : edge.created_at.toISOString()) : new Date().toISOString());
                if (result.changes > 0) {
                    insertedCount++;
                }
            }
 ...
 */
function insertRegionMetadata(db, metadata, dbPath) {
  ...
  `);
    insertStmt.run(metadata.region || null, metadata.total_trails || 0, metadata.total_nodes || 0, metadata.total_edges || 0, metadata.total_routes || 0, metadata.bbox_min_lat || null, metadata.bbox_max_lat || null, metadata.bbox_min_lng || null, metadata.bbox_max_lng || null, metadata.created_at ? (typeof metadata.created_at === 'string' ? metadata.created_at : metadata.created_at.toISOString()) : new Date().toISOString(), metadata.updated_at ? (typeof metadata.updated_at === 'string' ? metadata.updated_at : metadata.updated_at.toISOString()) : new Date().toISOString());
    console.log('[SQLITE] Inserted region metadata successfully.');
}
/**
 * Build region metadata object from trails data.
 */
function buildRegionMeta(trails, regionName, bbox) {
 ...
 */
function insertSchemaVersion(db, version, description, dbPath) {
 ...
 */
function getSchemaVersionFromDatabase(db) {
    try {
...
function insertRouteRecommendations(db, recommendations) {
    ...
    const insertMany = db.transaction((recommendations) => {
        for (const rec of recommendations) {
            try {
                    ...
                    if (rec.route_path) {
                        ...
                        if (routePath.coordinates && Array.isArray(routePath.coordinates)) {
                            ...
                            if (elevations.length > 0) {
                                ...
                                routeAvgElevation = elevations.reduce((sum, elev) => sum + elev, 0) / elevations.length;
                            }
                        }
                    }
                }
                ...
                if (routeMaxElevation <= 0)
                    routeMaxElevation = 1600; // Default fallback
                if (routeMinElevation <= 0)
                    routeMinElevation = 1500; // Default fallback  
                if (routeAvgElevation <= 0)
                    routeAvgElevation = 1550; // Default fallback
                // Calculate route difficulty based on gain rate
                let routeDifficulty = 'easy';
                if (routeGainRate >= 150)
                    routeDifficulty = 'expert';
                else if (routeGainRate >= 100)
                    routeDifficulty = 'hard';
                else if (routeGainRate >= 50)
                ...
                if (routeEstimatedTimeHours <= 0)
                    routeEstimatedTimeHours = 1.0; // Default fallback
...
function insertRouteTrails(db, routeTrails) {
    ...
    const existingTrailIds = new Set(db.prepare('SELECT app_uuid FROM trails').all().map((row) => row.app_uuid));
  ...
  `);
    const insertMany = db.transaction((routeTrails) => {
        for (const rt of routeTrails) {
            try {
                insertStmt.run(rt.route_uuid, rt.trail_id, rt.trail_name, rt.segment_order, rt.segment_distance_km, rt.segment_elevation_gain, rt.segment_elevation_loss, rt.created_at ? (typeof rt.created_at === 'string' ? rt.created_at : rt.created_at.toISOString()) : new Date().toISOString());
            }
            catch (error) {
...
//# sourceMappingURL=sqlite-export-helpers.js.map