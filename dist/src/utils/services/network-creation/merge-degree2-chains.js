...
exports.analyzeDegree2Chains = analyzeDegree2Chains;
exports.mergeDegree2Chains = mergeDegree2Chains;
const edge_composition_tracking_1 = require("./edge-composition-tracking");
/**
 ...
 */
async function analyzeDegree2Chains(pgClient, stagingSchema) {
    ...
    `);
        ...
        const edgesThatWouldBeRemoved = new Set();
        chainsThatWouldBeCreated.forEach(chain => {
            chain.edgeIds.forEach((edgeId) => edgesThatWouldBeRemoved.add(edgeId));
        });
 ...
 */
async function mergeDegree2Chains(pgClient, stagingSchema, toleranceMeters = 5.0) {
    ...
    `);
      ...
      `);
        ...
        if (chainsMerged > 0) {
            ...
            // Since we now use proper UUIDs, we need to identify merged edges differently
            // We'll use the merged_from_edges column to identify merged edges
            const mergedEdges = await pgClient.query(`
        SELECT id, app_uuid, merged_from_edges
        FROM ${stagingSchema}.ways_noded 
        WHERE merged_from_edges IS NOT NULL AND array_length(merged_from_edges, 1) > 1
        ORDER BY id DESC 
        LIMIT ${chainsMerged}
      `);
            for (const mergedEdge of mergedEdges.rows) {
                // Use the merged_from_edges column directly since we now use proper UUIDs
                if (mergedEdge.merged_from_edges && mergedEdge.merged_from_edges.length > 1) {
