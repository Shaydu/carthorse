...
exports.KspRouteGenerator = void 0;
class KspRouteGenerator {
        ...
        for (const recommendation of recommendations) {
            ...
            if (recommendation.route_path && recommendation.route_path.coordinates) {
                const elevations = recommendation.route_path.coordinates
                    .map((coord) => coord[2]) // Z coordinate (elevation)
                    .filter((elev) => elev !== null && elev !== undefined && !isNaN(elev));
            ...
            const routeDifficulty = (() => {
                if (recommendedElevationGain <= 0 || recommendedLengthKm <= 0)
                    return 'easy';
            ...
            const routeEstimatedTimeHours = (() => {
                if (recommendedLengthKm <= 0)
                    return 0;
                if (recommendedElevationGain <= 0)
                    return recommendedLengthKm / 4.0; // Default to easy pace
                const gainRate = recommendedElevationGain / recommendedLengthKm;
                if (gainRate >= 150)
                    return recommendedLengthKm / 2.0; // Expert: 2 km/h
                if (gainRate >= 100)
                    return recommendedLengthKm / 2.5; // Hard: 2.5 km/h
                if (gainRate >= 50)
                    return recommendedLengthKm / 3.0; // Moderate: 3 km/h
     ...
     */
    async generateOutAndBackRoutes(pattern, targetRoutes = 5) {
        ...
        `, [startNode, maxSearchDistance]);
            ...
            `, [startNode, endNode]);
                        ...
                        for (const [pathId, routeSteps] of routeGroups) {
                            ...
                            const edgeIds = routeSteps.map((step) => step.edge).filter((edge) => edge !== -1);
                            if (edgeIds.length === 0) {
     ...
     */
    async generateLoopRoutes(pattern, targetRoutes = 5) {
     ...
     */
    async generatePointToPointRoutes(pattern, targetRoutes = 5) {
    ...
    `);
        ...
        `, [startNode.id, endNode.id]);
                ...
                let totalDistance = 0;
                let totalElevationGain = 0;
                const edgeIds = dijkstraResult.rows.map((row) => row.edge).filter((edge) => edge !== -1);
                if (edgeIds.length === 0)
                    continue;
                const routeEdges = await this.pgClient.query(`
          SELECT * FROM ${this.stagingSchema}.ways_noded 
     ...
     */
    async generateWithPointsRoutes(pattern, targetRoutes = 5) {
    ...
    `, [pattern.target_distance_km]);
        ...
        `, [
                ...
                if (withPointsResult.rows.length === 0)
                    continue;
                // Calculate metrics
                let totalDistance = 0;
                let totalElevationGain = 0;
                const edgeIds = withPointsResult.rows.map((row) => row.edge).filter((edge) => edge !== -1);
                if (edgeIds.length === 0)
                    continue;
                const routeEdges = await this.pgClient.query(`
          SELECT * FROM ${this.stagingSchema}.ways_noded 
...
//# sourceMappingURL=ksp-route-generator.js.map