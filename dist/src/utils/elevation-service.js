...
const carthorse_postgres_atomic_insert_1 = require("../tools/carthorse-postgres-atomic-insert");
const geometry_parser_1 = require("./geometry-parser");
/**
...
class ElevationService {
    constructor(pgClient, enableTiffProcessing = false) {
        this.pgClient = pgClient;
        // Only create AtomicTrailInserter if TIFF processing is needed (data ingestion)
        if (enableTiffProcessing) {
            this.atomicInserter = new carthorse_postgres_atomic_insert_1.AtomicTrailInserter(process.env.PGDATABASE || 'trail_master_db_test');
        }
    }
    /**
     * Initialize elevation data for all trails (set to null by default)
     */
    async initializeElevationData(schemaName) {
     ...
     */
    async processMissingElevationData(schemaName) {
    ...
    `);
        ...
        for (const trail of trailsNeedingElevation.rows) {
            try {
                ...
                const coordinates = (0, geometry_parser_1.parseGeometryText)(trail.geometry_text);
     ...
     */
    async validateElevationData(schemaName) {
     ...
     */
    async getElevationStats(schemaName) {
