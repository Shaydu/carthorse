...
exports.RouteGenerationBusinessLogic = void 0;
const route_discovery_config_loader_1 = require("../../config/route-discovery-config-loader");
class RouteGenerationBusinessLogic {
    /**
     * Calculate tolerance levels for route generation
     */
    static getToleranceLevels(pattern) {
        try {
            const configLoader = route_discovery_config_loader_1.RouteDiscoveryConfigLoader.getInstance();
     ...
     */
    static isAreaUsed(startLon, startLat, usedAreas, minDistanceBetweenRoutes = 2.0) {
        return usedAreas.some(area => {
            const distance = Math.sqrt(Math.pow((startLon - area.lon) * 111.32 * Math.cos(startLat * Math.PI / 180), 2) +
     ...
     */
    static calculateRouteMetrics(routeEdges) {
        let totalDistance = 0;
     ...
     */
    static calculateOutAndBackMetrics(oneWayDistance, oneWayElevation) {
        return {
     ...
     */
    static meetsToleranceCriteria(outAndBackDistance, outAndBackElevation, pattern, tolerance) {
     ...
     */
    static calculateRouteScore(outAndBackDistance, outAndBackElevation, pattern, tolerance, routeEdges = []) {
     ...
     */
    static createRouteRecommendation(pattern, pathId, routeSteps, routeEdges, outAndBackDistance, outAndBackElevation, finalScore, region) {
     ...
     */
    static groupKspRouteSteps(kspRows) {
     ...
     */
    static extractEdgeIds(routeSteps) {
        return routeSteps.map((step) => step.edge).filter((edge) => edge !== -1);
    }
    /**
     * Calculate target metrics for out-and-back routes
     */
    static calculateTargetMetrics(pattern) {
        return {
     ...
     */
    static calculateDistanceToleranceRange(halfTargetDistance, tolerance) {
        return {
...
exports.RouteGenerationBusinessLogic = RouteGenerationBusinessLogic;
//# sourceMappingURL=route-generation-business-logic.js.map