...
class SQLiteExportStrategy {
    ...
    `);
        const insertMany = db.transaction((trails) => {
            for (const trail of trails) {
                ...
                insertTrails.run(trail.app_uuid, trail.name, trail.region, trail.osm_id, trail.trail_type, trail.surface_type, trail.difficulty, trail.geojson, trail.length_km, trail.elevation_gain, trail.elevation_loss, trail.max_elevation, trail.min_elevation, trail.avg_elevation, trail.bbox_min_lng, trail.bbox_max_lng, trail.bbox_min_lat, trail.bbox_max_lat);
            }
        });
    ...
    async exportRecommendations(db) {
      ...
      `);
            ...
            const processedRecommendations = await Promise.all(recommendationsResult.rows.map(async (rec) => {
            ...
            const insertMany = db.transaction((recommendations) => {
                for (const rec of recommendations) {
                    insertRecommendations.run(rec.route_uuid, this.config.region, rec.input_length_km, rec.input_elevation_gain, rec.recommended_length_km, rec.recommended_elevation_gain, rec.route_shape || 'unknown', rec.route_edges ? JSON.stringify(rec.route_edges) : null, rec.route_path ? JSON.stringify(rec.route_path) : null, rec.similarity_score);
                }
            });
    ...
    `);
        insertMetadata.run(this.config.region, stats.total_trails || 0, result.nodesExported, result.edgesExported, 0, // total_routes - not calculated yet
        stats.bbox_min_lng || 0, stats.bbox_max_lng || 0, stats.bbox_min_lat || 0, stats.bbox_max_lat || 0);
    }
    /**
    ...
    `);
        const insertMany = db.transaction((analyses) => {
            for (const analysis of analyses) {
                insertAnalysis.run(analysis.route_uuid, analysis.route_name, analysis.edge_count, analysis.unique_trail_count, analysis.total_trail_distance_km, analysis.total_trail_elevation_gain_m, analysis.out_and_back_distance_km, analysis.out_and_back_elevation_gain_m, JSON.stringify(analysis));
            }
        });
    ...
    async exportRouteTrails(db) {
      ...
      `);
            const insertMany = db.transaction((trails) => {
                for (const trail of trails) {
                    insertRouteTrail.run(trail.route_uuid, trail.trail_id, trail.trail_name, trail.segment_order, trail.segment_distance_km, trail.segment_elevation_gain, trail.segment_elevation_loss);
                }
            });
