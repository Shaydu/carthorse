...
class CarthorseOrchestrator {
    ...
    async detectAndFixGaps() {
        ...
        const validation = await (0, gap_detection_service_1.validateGapDetection)(this.pgClient, this.stagingSchema, gapConfig);
        ...
        const result = await (0, gap_detection_service_1.detectAndFixGaps)(this.pgClient, this.stagingSchema, gapConfig);
    ...
    async fillTrailGaps() {
        ...
        try {
            ...
            const { TrailEndpointConsolidationService } = await Promise.resolve().then(() => __importStar(require('../utils/services/network-creation/trail-endpoint-consolidation-service')));
    ...
    async deduplicateTrails() {
        ...
        try {
            const { TrailDeduplicationService } = await Promise.resolve().then(() => __importStar(require('../utils/services/network-creation/trail-deduplication-service')));
     ...
     */
    async mergeDegree2ChainsIteration() {
     ...
     */
    async measureNetworkConnectivity() {
        try {
            const result = await this.pgClient.query(`
     ...
     */
    async verifyNoOverlapsOrDegree2Chains() {
     ...
     */
    async iterativeNetworkOptimization() {
        ...
        while (iteration <= maxIterations) {
            ...
            const { runGapMidpointBridging } = await Promise.resolve().then(() => __importStar(require('../utils/services/network-creation/gap-midpoint-bridging')));
            const { getBridgingConfig } = await Promise.resolve().then(() => __importStar(require('../utils/config-loader')));
     ...
     */
    async replaceTrailsWithEnhancedSplitTrails() {
        console.log(`[ORCH] 📐 Replacing trails table with enhanced split trail segments...`);
        const result = await this.pgClient.query(`SELECT public.replace_trails_with_split_trails_enhanced($1, $2)`, [this.stagingSchema, 2.0] // Use default tolerance for now
        );
