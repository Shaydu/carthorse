-- Fixed copy function that properly deletes original trails before inserting split segments - V7
CREATE OR REPLACE FUNCTION copy_and_split_trails_to_staging_native_v7(
    staging_schema text, 
    source_table text, 
    region_filter text, 
...
BEGIN
    ...
    -- Step 1: Copy trails to staging first
    EXECUTE format($f$
    ...
    -- Step 2: Find intersections and split trails
    EXECUTE format($f$
        ...
        -- First, DELETE the original trails that are being split
    ...
    RETURN QUERY SELECT 
        ...
        true, 
        format('Successfully copied and split %s trails with %s intersections (v7)', split_count_var, intersection_count_var);

EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 0, 0, 0, false, format('Error during copy and split (v7): %s', SQLERRM);
END;
$$ LANGUAGE plpgsql; 