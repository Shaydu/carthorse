...
async function testComprehensiveIntersectionSplittingFixed() {
  try {
    console.log('🔍 Testing Comprehensive Intersection Splitting FIXED (X, Y, T, P)...');
...
async function testTrailPair(trail1Id, trail2Id, trail1Name, trail2Name) {
  ...
  if (intersectionInfo) {
    ...
    await testComprehensiveSplitting(trail1, trail2, intersectionInfo);
  }
}
...
async function exportResults() {
  try {
    ...
    const fs = require('fs');
    fs.writeFileSync('test-output/comprehensive-intersection-results-fixed.geojson', JSON.stringify(geojson, null, 2));
    console.log(`     📁 Results exported to test-output/comprehensive-intersection-results-fixed.geojson`);
...
testComprehensiveIntersectionSplittingFixed();
