...
async function exportImprovedSplittingResults() {
  let client;
  let testSchema;
  
  try {
    console.log('🚀 Exporting improved trail splitting results as GeoJSON...');
    ...
    // Create test schema
    testSchema = generateTestSchemaName('export_split_test');
    ...
    // Export original trails as GeoJSON
    console.log('\n📤 Exporting original trails...');
    ...
    const originalGeoJSON = {
      ...
      features: originalTrails.rows.map(row => ({
        type: "Feature",
        properties: {
          id: row.app_uuid,
          name: row.name,
    ...
    // Run improved trail splitting
    console.log('\n🔧 Running improved trail splitting...');
    ...
    // Export split trails as GeoJSON
    console.log('\n📤 Exporting split trails...');
    ...
    const splitGeoJSON = {
      type: "FeatureCollection",
      features: splitTrails.rows.map(row => ({
        type: "Feature",
        properties: {
          id: row.app_uuid,
          original_trail_uuid: row.original_trail_uuid,
          name: row.name,
          region: row.region,
          trail_type: row.trail_type,
          surface: row.surface,
          ...
          avg_elevation: row.avg_elevation,
          length_km: row.length_km,
          status: "split_segment"
        },
        geometry: JSON.parse(row.geometry_json)
      }))
    };
    
    // Export intersection point
    console.log('\n📤 Exporting intersection point...');
    ...
    let intersectionGeoJSON = null;
    if (intersectionPoint.rows.length > 0) {
      intersectionGeoJSON = {
        type: "FeatureCollection",
        features: [{
          type: "Feature",
          properties: {
            ...
            description: "Point where trails intersect and are split",
            status: "intersection"
          },
          geometry: JSON.parse(intersectionPoint.rows[0].intersection_geom)
        }]
      };
    }
    ...
    // Write original trails
    fs.writeFileSync(
      `${outputDir}/original-trails.geojson`,
      JSON.stringify(originalGeoJSON, null, 2)
    );
    console.log(`✅ Exported original trails to: ${outputDir}/original-trails.geojson`);
    
    // Write split trails
    fs.writeFileSync(
      `${outputDir}/split-trails.geojson`,
      JSON.stringify(splitGeoJSON, null, 2)
    );
    console.log(`✅ Exported split trails to: ${outputDir}/split-trails.geojson`);
    
    // Write intersection point
    if (intersectionGeoJSON) {
      fs.writeFileSync(
        `${outputDir}/intersection-point.geojson`,
        JSON.stringify(intersectionGeoJSON, null, 2)
      );
      console.log(`✅ Exported intersection point to: ${outputDir}/intersection-point.geojson`);
    }
    ...
    const combinedGeoJSON = {
      type: "FeatureCollection",
      features: [
        ...originalGeoJSON.features.map(f => ({ ...f, properties: { ...f.properties, layer: "original" } })),
        ...splitGeoJSON.features.map(f => ({ ...f, properties: { ...f.properties, layer: "split" } })),
        ...(intersectionGeoJSON ? intersectionGeoJSON.features.map(f => ({ ...f, properties: { ...f.properties, layer: "intersection" } })) : [])
      ]
    };
    
    fs.writeFileSync(
      `${outputDir}/improved-splitting-results.geojson`,
      JSON.stringify(combinedGeoJSON, null, 2)
    );
    console.log(`✅ Exported combined results to: ${outputDir}/improved-splitting-results.geojson`);
    
    // Print summary
    console.log('\n📊 Export Summary:');
    console.log(`   - Original trails: ${originalGeoJSON.features.length}`);
    console.log(`   - Split segments: ${splitGeoJSON.features.length}`);
    console.log(`   - Intersection points: ${intersectionGeoJSON ? intersectionGeoJSON.features.length : 0}`);
...
if (require.main === module) {
  exportImprovedSplittingResults().catch(console.error);
}

module.exports = { exportImprovedSplittingResults };
