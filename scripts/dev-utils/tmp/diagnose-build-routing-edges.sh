...
psql -d $DB <<EOF
\echo '--- Running build_routing_nodes and build_routing_edges ---'
DO \$
BEGIN
  BEGIN
    PERFORM $SCHEMA.build_routing_nodes('$SCHEMA', 'split_trails', 2.0);
    RAISE NOTICE 'build_routing_nodes ran successfully';
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'build_routing_nodes failed: %', SQLERRM;
  END;
  BEGIN
