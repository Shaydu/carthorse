...
async function debugRealGap() {
  ...
  try {
    ...
    const testSchema = 'debug_real_gap_' + Date.now();
    ...
    // Insert test data with a REAL gap - using coordinates that should create a gap
    // Enchanted Mesa Trail (from the JSON data)
    await client.query(`
      INSERT INTO ${testSchema}.trails (app_uuid, name, geometry) VALUES
      ('trail1', 'Enchanted Mesa Trail', ST_GeomFromText('LINESTRING(-105.2824 39.9886, -105.2820 39.9888, -105.2818 39.9890)', 4326)),
      ('trail2', 'Enchanted-Kohler Spur Trail', ST_GeomFromText('LINESTRING(-105.2822 39.9884, -105.2823931909462 39.98859709804337)', 4326))
    `);

    console.log('Test data created with real gap');
    ...
    for (const tolerance of [1.0, 2.0, 3.0, 5.0, 10.0]) {
...
debugRealGap().catch(console.error);
