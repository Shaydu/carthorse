...
const { TrailSplitter } = require('../src/utils/trail-splitter');
...
async function testTrailSplitting() {
  ...
  try {
    console.log('🚀 Starting standalone trail splitting test...');
    ...
    if (intersectionResult.rows.length === 0) {
      console.log('❌ No intersections detected! This might indicate a problem.');
      return;
    }
    
    // Test trail splitting
    console.log('\n🔧 Testing trail splitting with TrailSplitter...');
    ...
    const has3DCoordinates = splitSegments.rows.every(segment => segment.dimensions === 3);
    console.log(`\n✅ 3D coordinates preserved: ${has3DCoordinates ? 'YES' : 'NO'}`);
    ...
    `);
    console.log(`✅ Valid geometry segments: ${validGeometryCount.rows[0].count}/${result.finalCount}`);
    ...
    for (const tolerance of tolerances) {
      console.log(`\n   Testing tolerance: ${tolerance} meters`);
    ...
    console.log('\n✅ Trail splitting test completed successfully!');
...
module.exports = { testTrailSplitting };
