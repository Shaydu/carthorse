-- Aggressive cleanup script to remove old staging schemas
-- This processes schemas in very small batches and includes VACUUM
...
-- Drop schemas in batches of 5 to avoid memory limits
DO $$
DECLARE
...
BEGIN
    LOOP
        ...
        -- VACUUM after each batch to reclaim space
        RAISE NOTICE 'Running VACUUM to reclaim space...';
        VACUUM;
...
-- Final VACUUM to reclaim all space
VACUUM FULL;
