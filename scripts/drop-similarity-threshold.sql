-- Drop Minimum Similarity Score Requirement
-- Make route finding much more permissive for real trail data

-- 1. Update the minimum route score to be very low
CREATE OR REPLACE FUNCTION get_min_route_score() RETURNS float AS $$
BEGIN
    RETURN 0.01;  -- Drop from 0.3 (30%) to 0.01 (1%)
END;
$$ LANGUAGE plpgsql;

-- 2. Update the route finding function to be more permissive
...
-- 3. Test the changes
SELECT 'Minimum similarity score dropped from 30% to 1%' as info;
SELECT 'Route finding is now much more permissive:' as info;
SELECT '- Minimum distance: 100m (vs 5km)' as change;
SELECT '- Minimum elevation: 5m (vs 200m)' as change;
SELECT '- Distance tolerance: up to 500% of target' as change;
SELECT '- Elevation tolerance: up to 500% of target' as change;
SELECT '- Similarity threshold: 1% (vs 30%)' as change;
