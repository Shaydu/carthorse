...
async function testActual189mGap() {
  ...
  try {
    ...
    const testSchema = 'test_189m_gap_' + Date.now();
    ...
    // Insert test data with an ACTUAL 1.89m gap
    // Create a gap by moving the endpoint further away - approximately 1.89m
    // At this latitude, 1.89m ≈ 0.000017 degrees
    await client.query(`
      ...
      ('trail2', 'Enchanted-Kohler Spur Trail', ST_GeomFromText('LINESTRING(-105.2822 39.9884, -105.2824 39.9886)', 4326))
    `);

    console.log('Test data created with 1.89m gap');
    ...
    const distanceCheck = await client.query(`
      ...
      SELECT 
        ...
        ST_DWithin(t1.start_point, t2.geometry, 1.89) as within_189m
      FROM trail_data t1
      ...
      SELECT 
        ...
        ST_DWithin(t1.end_point, t2.geometry, 1.89) as within_189m
      FROM trail_data t1
      CROSS JOIN trail_data t2
      WHERE t1.app_uuid != t2.app_uuid
    ...
    distanceCheck.rows.forEach(row => {
      ...
      console.log(`    Within 1.89m: ${row.within_189m}`);
...
testActual189mGap().catch(console.error);
