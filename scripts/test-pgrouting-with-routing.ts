...
async function testPgRoutingWithRouting() {
  ...
  try {
    ...
    if (sampleNodesResult.rows.length >= 2) {
      ...
      console.log(`🎯 Testing routing from node ${startNode} to node ${endNode}...`);
      ...
      if (shortestPathResult.rows.length > 0) {
        console.log(`✅ Shortest path found with ${shortestPathResult.rows.length} segments`);
        console.log(`   Total cost: ${shortestPathResult.rows.reduce((sum, row) => sum + (row.cost || 0), 0)} meters`);
      } else {
        console.log(`❌ Shortest path failed: No route found`);
      }
      
      // Test k-shortest paths using pgr_ksp
      console.log('🛤️ Testing k-shortest paths...');
      ...
      if (kspResult.rows.length > 0) {
        console.log(`✅ K-shortest paths found: ${kspResult.rows.length} route segments across ${new Set(kspResult.rows.map(r => r.path_id)).size} routes`);
        ...
        console.log(`❌ K-shortest paths failed: No routes found`);
      }
      
      // Test route recommendations
      console.log('🎯 Testing route recommendations...');
      const recommendationsResult = await pgrouting.generateRouteRecommendations(5.0, 500, 5);
      if (recommendationsResult.success) {
        console.log(`✅ Route recommendations generated: ${recommendationsResult.routes?.length || 0} routes`);
      } else {
        console.log(`❌ Route recommendations failed: ${recommendationsResult.error}`);
      }
    } else {
      console.log('⚠️ Not enough nodes for routing tests');
    }
