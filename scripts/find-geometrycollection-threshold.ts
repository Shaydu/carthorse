...
/**
 * Find GeometryCollection Error Threshold
 * 
 * This script tests pgr_nodeNetwork with different numbers of trails to find
 * the threshold where the GeometryCollection error occurs
 */

import { Pool } from 'pg';

async function findGeometryCollectionThreshold() {
  console.log('🔍 Finding GeometryCollection error threshold...');
  ...
  try {
    ...
    const thresholds = [10, 25, 50, 100, 200, 500, 1000, 1500, 2000, totalCount];
    
    console.log('🔄 Step 2: Testing different trail count thresholds...');
    ...
    for (const threshold of thresholds) {
      ...
      try {
        // Create a temporary ways table with limited trails
        ...
        if (errorMessage.includes('GeometryCollection')) {
          ...
          const filename = `geometrycollection-error-threshold-${threshold}.geojson`;
    ...
    if (firstFailureThreshold) {
      console.log(`❌ GeometryCollection error occurs between ${lastSuccessfulThreshold} and ${firstFailureThreshold} trails`);
      ...
      if (exactThreshold) {
        ...
        console.log(`❌ ${exactThreshold} trails: GEOMETRYCOLLECTION ERROR`);
      }
    ...
    console.log('✅ GeometryCollection threshold analysis complete!');
...
// Run the analysis
findGeometryCollectionThreshold()
  .then(() => {
    console.log('🎉 GeometryCollection threshold analysis completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('💥 GeometryCollection threshold analysis failed:', error);
    process.exit(1);
  }); 