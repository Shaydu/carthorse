...
async function testLongerMesaTrail() {
  ...
  try {
    ...
    const testSchema = 'test_longer_mesa_' + Date.now();
    ...
    // Insert test data with a longer Enchanted Mesa Trail so the snapped point falls in the middle
    await client.query(`
      INSERT INTO ${testSchema}.trails (app_uuid, name, geometry) VALUES
      ('trail1', 'Enchanted Mesa Trail', ST_GeomFromText('LINESTRING(-105.2830 39.9880, -105.2828 39.9881, -105.2826 39.9882, -105.2824 39.9886, -105.2822 39.9887, -105.2820 39.9888, -105.2818 39.9889, -105.2816 39.9890, -105.2814 39.9891)', 4326)),
      ('trail2', 'Enchanted-Kohler Spur Trail', ST_GeomFromText('LINESTRING(-105.2822 39.9884, -105.282387 39.988581)', 4326))
    `);

    console.log('Test data created with longer Enchanted Mesa Trail');
...
testLongerMesaTrail().catch(console.error);
