...
async function testSpurSplittingMainTrail() {
  ...
  try {
    ...
    const testSchema = 'test_spur_split_' + Date.now();
    ...
    // Insert test data where the spur trail endpoint is in the middle of the main trail
    await client.query(`
      ...
      ('trail2', 'Enchanted-Kohler Spur Trail', ST_GeomFromText('LINESTRING(-105.2822 39.9884, -105.2822 39.9887)', 4326))
    `);

    console.log('Test data created - spur endpoint in middle of main trail');
...
testSpurSplittingMainTrail().catch(console.error);
