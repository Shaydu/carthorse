...
async function generateKspRoutes() {
  ...
  try {
    ...
    // Step 6.5.5: Fix connectivity issues by connecting nearby edges and endpoints
    console.log('🔗 Fixing connectivity issues...');
    
    const connectionTolerance = 0.000045; // ~5 meters in degrees (5m / 111320 m/degree)
    
    // 1. Connect edges that come within 5m of nodes but aren't properly connected
    const edgeToNodeConnections = await pool.query(`
      WITH nearby_edges_nodes AS (
        ...
        CROSS JOIN ${stagingSchema}.ways_noded_vertices_pgr n
    ...
    const endpointConnections = await pool.query(`
      ...
      for (const tolerance of toleranceLevels) {
        ...
        for (let i = 0; i < Math.min(nodesResult.rows.length - 1, 10); i++) {
          ...
          try {
            ...
            // Debug: Check what fields KSP is returning
            if (kspResult.rows.length > 0) {
              console.log(`🔍 First route fields: ${Object.keys(kspResult.rows[0]).join(', ')}`);
              console.log(`🔍 First route data:`, kspResult.rows[0]);
            }
            
            // Debug: Check what edge IDs exist in ways_noded
            const edgeIdsResult = await pool.query(`
              SELECT id FROM ${stagingSchema}.ways_noded ORDER BY id LIMIT 10
            `);
            console.log(`🔍 Available edge IDs: ${edgeIdsResult.rows.map(r => r.id).join(', ')}`);
