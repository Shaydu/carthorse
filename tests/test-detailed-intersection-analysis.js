...
async function analyzeIntersectionDistances() {
  ...
  try {
    ...
    // Detailed analysis of intersection points and their distances from endpoints
    const detailedAnalysisQuery = `
      ...
      SELECT 
        ...
        -- Check if this is near any endpoint (within 1 meter)
        CASE 
    ...
    console.log('Each intersection point with distances from all endpoints:\n');
    ...
    for (const row of result.rows) {
      ...
      console.log(`   Classification: ${row.intersection_location}`);
      console.log('---');
    }

    // Now test with different tolerance values
    ...
    const toleranceTestQuery = `
      ...
      SELECT 
        ...
        ) as has_endpoint_intersection_0_1m,
        EXISTS(
          SELECT 1 FROM (
            SELECT (ST_Dump(intersection_geom)).geom as point_geom
          ) points
        ...
        ) as has_endpoint_intersection_1_0m,
        EXISTS(
          SELECT 1 FROM (
            SELECT (ST_Dump(intersection_geom)).geom as point_geom
          ) points
        ...
        ) as has_middle_intersection_0_1m,
        EXISTS(
          SELECT 1 FROM (
            SELECT (ST_Dump(intersection_geom)).geom as point_geom
          ) points
        ...
        ) as has_middle_intersection_1_0m
      FROM intersection_analysis;
    `;

    const toleranceResult = await client.query(toleranceTestQuery);
    
    for (const row of toleranceResult.rows) {
      console.log(`Trails: ${row.trail1_name} ↔ ${row.trail2_name}`);
      console.log(`Point Count: ${row.point_count}`);
      console.log(`With 0.1m tolerance:`);
      console.log(`  Has endpoint intersection: ${row.has_endpoint_intersection_0_1m}`);
      console.log(`  Has middle intersection: ${row.has_middle_intersection_0_1m}`);
      console.log(`With 1.0m tolerance:`);
      console.log(`  Has endpoint intersection: ${row.has_endpoint_intersection_1_0m}`);
      console.log(`  Has middle intersection: ${row.has_middle_intersection_1_0m}`);
      console.log('---');
    }
