...
async function testToleranceIssue() {
  ...
  try {
    ...
    // Test different tolerance values for middle intersection detection
    const toleranceTestQuery = `
      WITH trail_pairs AS (
        SELECT 
      ...
      SELECT 
        ...
        CASE 
          WHEN ST_DWithin(point_geom, trail1_start, 0.1) THEN 'near_trail1_start_0.1m'
          WHEN ST_DWithin(point_geom, trail1_end, 0.1) THEN 'near_trail1_end_0.1m'
          WHEN ST_DWithin(point_geom, trail2_start, 0.1) THEN 'near_trail2_start_0.1m'
          WHEN ST_DWithin(point_geom, trail2_end, 0.1) THEN 'near_trail2_end_0.1m'
          ELSE 'middle_intersection_0.1m'
        END as classification_0_1m,
        CASE 
          WHEN ST_DWithin(point_geom, trail1_start, 1.0) THEN 'near_trail1_start_1.0m'
          WHEN ST_DWithin(point_geom, trail1_end, 1.0) THEN 'near_trail1_end_1.0m'
          WHEN ST_DWithin(point_geom, trail2_start, 1.0) THEN 'near_trail2_start_1.0m'
          WHEN ST_DWithin(point_geom, trail2_end, 1.0) THEN 'near_trail2_end_1.0m'
          ELSE 'middle_intersection_1.0m'
        END as classification_1_0m,
        CASE 
          WHEN ST_DWithin(point_geom, trail1_start, 5.0) THEN 'near_trail1_start_5.0m'
          WHEN ST_DWithin(point_geom, trail1_end, 5.0) THEN 'near_trail1_end_5.0m'
          WHEN ST_DWithin(point_geom, trail2_start, 5.0) THEN 'near_trail2_start_5.0m'
          WHEN ST_DWithin(point_geom, trail2_end, 5.0) THEN 'near_trail2_end_5.0m'
          ELSE 'middle_intersection_5.0m'
        END as classification_5_0m
      FROM intersection_points
      ORDER BY trail1_name, trail2_name, point_path;
    `;

    const result = await client.query(toleranceTestQuery);
    
    console.log('\n=== TOLERANCE ANALYSIS ===');
    console.log('Testing different tolerance values for middle intersection detection:\n');
    
    for (const row of result.rows) {
      console.log(`📍 Intersection Point: ${row.point_wkt}`);
      console.log(`   Coordinates: ${row.lon}, ${row.lat}`);
      ...
      console.log(`   Classification with 0.1m tolerance: ${row.classification_0_1m}`);
      console.log(`   Classification with 1.0m tolerance: ${row.classification_1_0m}`);
      console.log(`   Classification with 5.0m tolerance: ${row.classification_5_0m}`);
      console.log('---');
    }

    // Now test the current MultipointIntersectionSplittingService logic
    console.log('\n=== CURRENT MULTIPOINT SERVICE LOGIC ===');
    const currentLogicQuery = `
      ...
      intersection_analysis AS (
        SELECT 
          ...
          ST_NumGeometries(ST_Force3D(ST_Intersection(trail1_geom, trail2_geom))) as point_count,
          trail1_start,
          trail1_end,
          trail2_start,
          trail2_end,
          -- Current logic: has_middle_intersection = points NOT within 0.1m of endpoints
          EXISTS(
            SELECT 1 FROM (
              SELECT (ST_Dump(ST_Force3D(ST_Intersection(trail1_geom, trail2_geom)))).geom as point_geom
            ) points
          ...
          ) as has_middle_intersection_current,
          -- Proposed logic: has_middle_intersection = points within 5m but not within 0.1m of endpoints
          EXISTS(
            SELECT 1 FROM (
              SELECT (ST_Dump(ST_Force3D(ST_Intersection(trail1_geom, trail2_geom)))).geom as point_geom
            ) points
            WHERE (ST_DWithin(points.point_geom, trail1_start, 5.0) 
              AND NOT ST_DWithin(points.point_geom, trail1_start, 0.1))
              OR (ST_DWithin(points.point_geom, trail1_end, 5.0) 
              AND NOT ST_DWithin(points.point_geom, trail1_end, 0.1))
              OR (ST_DWithin(points.point_geom, trail2_start, 5.0) 
              AND NOT ST_DWithin(points.point_geom, trail2_start, 0.1))
              OR (ST_DWithin(points.point_geom, trail2_end, 5.0) 
              AND NOT ST_DWithin(points.point_geom, trail2_end, 0.1))
          ) as has_middle_intersection_proposed
        FROM trail_pairs
        WHERE ST_GeometryType(ST_Intersection(trail1_geom, trail2_geom)) = 'ST_MultiPoint'
      )
      SELECT 
        ...
        trail2_name,
        point_count,
        has_middle_intersection_current,
        has_middle_intersection_proposed,
        CASE 
          WHEN point_count = 2 AND has_middle_intersection_current THEN 'x_intersection_CURRENT'
          WHEN point_count = 2 AND NOT has_middle_intersection_current THEN 'endpoint_intersection_CURRENT'
          ELSE 'other_CURRENT'
        END as intersection_type_current,
        CASE 
          WHEN point_count = 2 AND has_middle_intersection_proposed THEN 'x_intersection_PROPOSED'
          WHEN point_count = 2 AND NOT has_middle_intersection_proposed THEN 'endpoint_intersection_PROPOSED'
          ELSE 'other_PROPOSED'
        END as intersection_type_proposed
      FROM intersection_analysis;
    `;

    const currentLogicResult = await client.query(currentLogicQuery);
    
    for (const row of currentLogicResult.rows) {
      console.log(`Trails: ${row.trail1_name} ↔ ${row.trail2_name}`);
      console.log(`Point Count: ${row.point_count}`);
      console.log(`Current Logic:`);
      console.log(`  has_middle_intersection: ${row.has_middle_intersection_current}`);
      console.log(`  intersection_type: ${row.intersection_type_current}`);
      console.log(`Proposed Logic:`);
      console.log(`  has_middle_intersection: ${row.has_middle_intersection_proposed}`);
      console.log(`  intersection_type: ${row.intersection_type_proposed}`);
      console.log('---');
    }
